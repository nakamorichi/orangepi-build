...
compile_firmware()
{
	...
	[[ $IGNORE_UPDATES != yes ]] && fetch_from_repo "https://github.com/nakamorichi/firmware" "${EXTER}/cache/sources/orangepi-firmware-git" "branch:master"
	if [[ -n $FULL ]]; then
...
userpatch_create()
{
	...
	if ! git diff-index --quiet --cached HEAD; then
		...
		if [[ -n $(git config user.email) ]]; then
			...
			git format-patch -1 HEAD --stdout --signature="Created with orangepi build tools https://github.com/nakamorichi/orangepi-build" > "${patch}"
